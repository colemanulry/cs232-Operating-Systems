# File edited by Coleman Ulry
# CS 232
# added functionality of the timer_sir(), context_switch(), and run() methods
# 3-11-21
...
class CalOS:
    ...
    def timer_isr(self):
        ...
        '''
        # save cpu registers into pcb
        self.current_proc.set_registers(self._cpu.get_registers())

        # if ready queue is not empty, context switch
        if self._ready_q:
            self.context_switch()
    ...
    def context_switch(self):
        ...
        # get pcb off front of ready queue
        new_proc = self._ready_q.pop(0)

        # save registers into currently running process pcb
        self.current_proc.set_registers(self._cpu.get_registers())

        # load registers with new pcb registers
        self._cpu.set_registers(new_proc.get_registers())

        # set state of current process to ready
        self.current_proc.set_state(PCB.READY)

        # put old process on end of ready queue
        self._ready_q.append(self.current_proc)
    ...
    def run(self):
        ...
        while self._ready_q:
            ...
            # pop first item off ready queue and set it to temp process
            temp_proc = self._ready_q.pop(0)

            # set current process to result of pop
            self.current_proc = temp_proc

            # reset timer
            self.reset_timer()

            # load registers with current_proc registers
            self._cpu.set_registers(self.current_proc.get_registers())

            # call run_process method
