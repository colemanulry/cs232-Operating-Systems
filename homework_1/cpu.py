...
@date 12/26/17
@editor Coleman Ulry (ctu2)
@date edited 2/11/21
additions - _batch_mode variable, _set_batch method, altered run() function to allow batch mode using simple if-else
'''
...
class CPU(threading.Thread):
    ...
    # set batch mode to false - to be toggled to true when "r" is inputted
    _batch_mode = False
    ...
    # batch mode - setter method
    def set_batch(self, batch):
        # returns true if batch is a boolean, either true or false
        assert isinstance(batch, bool)
        # set batch variable to this value
    ...
    def run(self):
        # if batch mode variable is true, run this code
        if self._batch_mode:
            # store program counter in ram - start address
            self._ram[0] = self._registers['pc']
            # set program counter to start address
            program_addr = self._ram[self._ram[0]]

            # while loop will call run program on each program in the list
            # while loop will terminate when a 0 is encountered
            while self._ram[self._ram[0]] != 0:
                # set pc register to address of program
                self._registers['pc'] = program_addr
                # call run program function
                self._run_program()
                # increment
                self._ram[0] += 1
                # set new program address, which was just incremented above
                program_addr = self._ram[self._ram[0]]
        # else, if batch variable is false, run program - non batch mode
        else:
            self._run_program()
